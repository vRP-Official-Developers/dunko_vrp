...
function Tunnel.bindInterface(name, interface)
    ...
    AddEventHandler(name .. ":tunnel_req", function(member, args, identifier, rid)
        ...
        if Debug.active then
            Debug.pbegin("tunnelreq#" .. rid .. "_" .. name .. ":" .. member .. " " ..
                             json.encode(Debug.safeTableCopy(args)))
        end
        ...
        if type(f) == "function" then
            ...
            if (type(args)) == "table" then
                rets = {f(table.unpack(args))} -- call function with and return values through callback
                -- CancelEvent() -- cancel event doesn't seem to cancel the event for the other handlers, but if it does, uncomment this
            else
                rets = callback({f(args)})
            end
            -- CancelEvent() -- cancel event doesn't seem to cancel the event for the other handlers, but if it does, uncomment this
        end
...
function Tunnel.getInterface(name, identifier)
    ...
    AddEventHandler(name .. ":" .. identifier .. ":tunnel_res", function(rid, args)
        ...
        if callback ~= nil then
            ...
            -- call
            if (type(args) == "table") then
                callback({table.unpack(args)}) -- call function with and return values through callback
                -- CancelEvent() -- cancel event doesn't seem to cancel the event for the other handlers, but if it does, uncomment this
            else 
                callback({args}) 
            end
