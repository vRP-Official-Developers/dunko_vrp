...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.ghmattimysql.execute("SELECT * FROM `vrp_warnings` WHERE user_id = ?", [params[0]], (result) => {
        ...
        table.setHeading('Warning Type', 'Duration', 'Reason', 'Admin', 'Date')
        for (i = 0; i < result.length; i++) {
            var date = new Date(+result[i].warning_date)
            table.addRow(result[i].warning_type, result[i].duration, result[i].reason, result[i].admin, date.toDateString())
        }
