...
PerformHttpRequest("https://raw.githubusercontent.com/DunkoUK/dunko_vrp/master/vrp_discordbot/version.lua",function(err,text,headers)
if err == 200 then
    text = string.gsub(text,"return ","")
    local r_version = tonumber(text)
    if version ~= r_version then
        print("^5[VRP]: ^7" .. 'A vRP Discord Bot Update is available from: https://github.com/DunkoUK/dunko_vrp')
    else 
        print("^5[VRP]: ^7" .. 'You are running the most up to date JamesUK vRP Bot Version. Thanks for using Jam_e_s_U_K bot and thanks to our contributors for updating the project. Support Found At: https://discord.gg/b8wQn2XqDt')
    end
else
